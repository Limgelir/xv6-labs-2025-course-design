...
write_acquire_inner(struct rwspinlock *rwlk)
{
  ...
  if(rwlk->waiting_writers == 0)
    rwlk->writer_pending = 0;
...
sys_rwlktest()
{
  ...
  rwspinlock_test_step(++step,
      "concurrent read_acquire");
  ...
  rwspinlock_test_step(++step,
      "prepare read_acquire for writer priority test");
  ...
  if(id == 2){
    ...
    if(f==0){
      ...
      printf(
      "rwspinlock_test: reader sneaked ahead of waiting writer\n");
  ...
  rwspinlock_test_step(++step,
      "checking for concurrent readers/writers");
  ...
  if(id==0){
    ...
    for(int i=0;i<1000000;i++){
      ...
      uint x =
      __atomic_add_fetch(&v,1,__ATOMIC_ACQ_REL);
      ...
      uint y =
      __atomic_fetch_sub(&v,1,__ATOMIC_ACQ_REL);
    ...
    if(maxwv>1){
      ...
      printf(
      "rwspinlock_test: cpu %d saw concurrent reads/writes\n",
      id);
    ...
    if(maxrv<2){
      ...
      printf(
      "rwspinlock_test: cpu %d never saw concurrent reads\n",
      id);
  ...
  rwspinlock_test_step(++step,
      "checking for concurrent writers");
  ...
  if(maxwv>1){
    ...
    printf(
    "rwspinlock_test: cpu %d saw concurrent writes\n",
    id);
